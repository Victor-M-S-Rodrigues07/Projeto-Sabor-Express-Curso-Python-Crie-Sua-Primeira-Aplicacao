# <h1 align = "center"> Sabor Express - Aplicativo de Restaurantes </h1>

<div align = "center">
<img src = ".\imgs\readme-capa.png" alt = "Imagem do aplicativo 'Sabor Express', com os códigos atrás" height = "600">
</div>

### Status do Projeto: Concluído ✔

## 🗂 Sumário

- [📝 Descrição do Projeto](https://github.com/Victor-M-S-Rodrigues07/Projeto-Alura-Plus--Projeto-N1-2026#-descri%C3%A7%C3%A3o-do-projeto)
- [🎯 Objetivos do Curso](https://github.com/Victor-M-S-Rodrigues07/Projeto-Alura-Plus--Projeto-N1-2026#-objetivos-do-curso)
- [🔗 Protótipo do Projeto no Figma](https://github.com/Victor-M-S-Rodrigues07/Projeto-Alura-Plus--Projeto-N1-2026#-prot%C3%B3tipo-do-projeto-no-figma)
- [💻 Tecnologias Utilizadas](https://github.com/Victor-M-S-Rodrigues07/Projeto-Alura-Plus--Projeto-N1-2026#-tecnologias-utilizadas)
- [⚙ Funcionalidades e Inovações do Projeto](https://github.com/Victor-M-S-Rodrigues07/Projeto-Alura-Plus--Projeto-N1-2026#-funcionalidades-e-inova%C3%A7%C3%B5es-do-projeto)
- [📂 Estrutura do Projeto](https://github.com/Victor-M-S-Rodrigues07/Projeto-Alura-Plus--Projeto-N1-2026#-estrutura-do-projeto)
- [📷 Imagens do Projeto](https://github.com/Victor-M-S-Rodrigues07/Projeto-Alura-Plus--Projeto-N1-2026#-imagens-do-projeto)
- [✨Autor](https://github.com/Victor-M-S-Rodrigues07/Projeto-Alura-Plus--Projeto-N1-2026#-autor)
- [☎ Contatos](https://github.com/Victor-M-S-Rodrigues07/Projeto-Alura-Plus--Projeto-N1-2026#-contatos)

## 📝 Descrição do Projeto

A Sabor Express é uma plataforma de cadastro de restaurantes, na qual o usuário pode adicionar a sua loja de comida na lista de restaurantes disponíveis para receber pedidos. Com isso, os usuários podem acessar o perfil da loja e fazer as suas solicitação em um menu dinâmico, simples e fácil de compreender. Com funções bem definidas e mensagens claras para guiar os usuários através da aplicação, o projeto retrata um exemplo comum de aplicativo, com as mesmas práticas, interfaces, propósios e métodos usados no mercado de trabalho da tecnologia. <br>

Cada função do código exerce sua atribuição conferida, garantindo maior modulação, seguindo as boas práticas de programação. Além disso, o código conta com todos os métodos documentados, utilizando a DocStrings do Python. Também nessa aplicação foi aplicado o conceito de dicionários com o objetivo de registrar os dados de forma mais organizada. Por fim, foram criados blocos de repetição com o *for* para iterações com listas e impressões, garantindo otimização e um código mais limpo, seguindo os princípos do *Clear Code*.<br>

Esse projeto foi criado a fim de iniciar o aprendizado da linguagem de programação *Python*, apresentada no curso *Python: Crie a sua primeira aplicação* da Alura (o link do curso estará disponível nos créditos abaixo). <br>

## 🎯 Objetivos do Curso

- Crie um projeto em Python usando o VSCode;
- Descubra o fluxo de uma aplicação com o uso de condicionais e laços de repetição;
- Aprenda a utilizar blocos de controle de execução try-;
- Crie funções para mostrar o menu principal e registrar restaurantes em listas e dicionários <br>

## 💻 Tecnologias Utilizadas

![Python](https://img.shields.io/badge/Python-0?style=for-the-badge&logo=python&logoColor=%23FFFFFF&color=%233776AB)
![Git](https://img.shields.io/badge/Git-0?style=for-the-badge&logo=git&logoColor=%23FFFFFF&color=%23F05032)
![GitHub](https://img.shields.io/badge/GitHub-0?style=for-the-badge&logo=github&logoColor=%23FFFFFF&color=%23181717) <br>

## ⚙ Funcionalidades do Projeto

- Uso de funções em *Python* para modularização do projeto;
- Laço de Repetição *For* para iterar listas;
- Uso de condicionais *if/elif/else* para controle de fluxo;
- *Try-Except* para validações e previsão de erros;
- DocString para comunicação e explicação de funções entre a equipe. <br>

## 📂 Estrutura do Projeto

```
.gitignore

/exercicios-de-pratica
    exercicios-p1.py
    exercicios-p2.py
    exercicios-p3.py
    exercicios-p4.py

/imgs

app.py
README.md
```

> [!NOTE]
> Achou-se por bem não retirar a pasta com os exercícios de prática do curso com o propóstio de permitir uma melhor análise da evolução do aprendizado em relação ao *Python* e às boas práticas de programação.

## ✨ Autor

<img src = "https://avatars.githubusercontent.com/u/187053289?v=4" width = 120px> <br>
<strong> Victor </strong>

## ☎ Contatos

<a href = "https://www.linkedin.com/in/victor-m-rodrigues/"><img alt="Static Badge" src="https://img.shields.io/badge/LinkedIn-0?style=for-the-badge&logoColor=%23FFFFFF&color=%230077B5&link=https%3A%2F%2Fwww.linkedin.com%2Fin%2Fvictor-m-rodrigues%2F"></a> 
<a href = "https://github.com/Victor-M-S-Rodrigues07/"><img alt="Static Badge" src="https://img.shields.io/badge/GitHub-0?style=for-the-badge&logo=github&logoColor=%23ffffff&labelColor=%23000000&color=%2300000000"></a>
<a href = "https://cursos.alura.com.br/user/victorvicmr"><img alt="Static Badge" src="https://img.shields.io/badge/Alura-0?style=for-the-badge&color=%23100D36"></a>
<a href = "https://cursos.alura.com.br/course/python-crie-sua-primeira-aplicacao"><img alt="Static Badge" src="https://img.shields.io/badge/Python%3A%20Crie%20a%20sua%20primeira%20aplica%C3%A7%C3%A3o-0?style=for-the-badge&label=Curso&labelColor=Black&color=FF6E6C">
</a>

🇧🇷 - 2026
